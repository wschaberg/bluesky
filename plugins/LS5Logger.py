...
def init_plugin():
    ...
    first_locframe = True
    headings = [5,7.5,10,15,20,25,30,35,40,45]
    ...
    i = 1
    superfirst = True
    resos = ['MVP','MVPFTR','MVPGON','OFF']
    reso_index = 0
    # Configuration parameters
    config = {
        # The name of your plugin
        'plugin_name':     'LS5Logger',
...
def update():
    ...
    if recording:                                                                                               # Omgekeerd Statement, de not moet eigenlijk weg.
        ...
        if sim.simt > 0.1*3600 and not first_confframe and not first_delframe: # Moet 4 ofzo zijn
            if not os.path.exists('/BSData/Dens'+str(densities[density_index])+'/Head'+str(headings[heading_index])):
                os.makedirs('/BSData/Dens'+str(densities[density_index])+'/Head'+str(headings[heading_index]), exist_ok=True)

            #pd.DataFrame(confframe).to_csv(str(headings[heading_index])+'_'+str(densities[density_index])+'_'+str(i)+'_'+reso+'_ResultsDF.csv', sep=';')
            pd.DataFrame(confframe).to_csv('/BSData/Dens'+str(densities[density_index])+'/Head'+str(headings[heading_index])+'/'+str(i)+'-'+resos[reso_index]+'-confframe.csv' ,sep=';')
            ...
            pd.DataFrame(delframe).to_csv('/BSData/Dens'+str(densities[density_index])+'/Head'+str(headings[heading_index])+'/'+str(i)+'-'+resos[reso_index]+'-delframe.csv' ,sep=';')
            ...
            pd.DataFrame(ntrafframe).to_csv('/BSData/Dens'+str(densities[density_index])+'/Head'+str(headings[heading_index])+'/'+str(i)+'-'+resos[reso_index]+'-ntrafframe.csv' ,sep=';')
            ...
            pd.DataFrame(locframe).to_csv('/BSData/Dens'+str(densities[density_index])+'/Head'+str(headings[heading_index])+'/'+str(i)+'-'+resos[reso_index]+'-locframe.csv' ,sep=';')
    ...
    for aircraft in traf.id:
        if traf.actwp.lon[traf.id2idx(aircraft)] > 10:
            if first_delframe:
                ...
                delframe = np.vstack((delframe, np.array([sim.simt, aircraft, traf.lat[traf.id2idx(aircraft)], traf.lat[traf.id2idx(aircraft)], traf.distflown[traf.id2idx(aircraft)]])))                                                      
